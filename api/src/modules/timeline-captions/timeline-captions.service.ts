...
export class TimelineCaptionsService {
  ...
  async create(user_uuid: string, dto: CreateTimelineCaptionDto) {
    try {
      ...
      throw new InternalServerErrorException('Failed to create caption', { cause: error });
    }
  }
  ...
  async findAll(user_uuid: string, query: TimelineCaptionQueryDto) {
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to retrieve captions', { cause: error });
    }
  }

  async findOne(user_uuid: string, uuid: string) {
    try {
      const caption = await this.prisma.timelineCaption.findUnique({ where: { uuid } });
      if (!caption) throw new NotFoundException('Caption not found');
      await this.assertClipOwnership(user_uuid, caption.clip_uuid);
      return caption;
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to retrieve caption', { cause: error });
    }
  }
  ...
  async update(user_uuid: string, uuid: string, dto: UpdateTimelineCaptionDto) {
    try {
      ...
      return await this.prisma.timelineCaption.update({ where: { uuid }, data: dto });
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to update caption', { cause: error });
    }
  }
  ...
  async remove(user_uuid: string, uuid: string) {
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to delete caption', { cause: error });
    }
  }
}
