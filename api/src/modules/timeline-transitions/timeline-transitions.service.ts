...
export class TimelineTransitionsService {
  ...
  async create(dto: CreateTimelineTransitionDto) {
    try {
      ...
      throw new InternalServerErrorException('Failed to create transition', { cause: error });
    }
  }

  async findOne(uuid: string) {
    try {
      const transition = await this.prisma.timelineTransition.findUnique({ where: { uuid } });
      if (!transition) throw new NotFoundException('Transition not found');
      return transition;
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to retrieve transition', { cause: error });
    }
  }
  ...
  async update(uuid: string, dto: UpdateTimelineTransitionDto) {
    try {
      ...
      return await this.prisma.timelineTransition.update({ where: { uuid }, data: dto });
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to update transition', { cause: error });
    }
  }
  ...
  async remove(uuid: string) {
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to delete transition', { cause: error });
    }
  }
}
