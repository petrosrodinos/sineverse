    ...
    async getBalance(): Promise<AimlBillingBalance> {
        ...
        try {
            const primary = await firstValueFrom(
                this.httpService.get('https://api.aimlapi.com/v2/billing', { headers, timeout: 15000 }),
            );
            ...
            const fallback = await firstValueFrom(
                this.httpService.get('https://api.aimlapi.com/v2/billing/detail', { headers, timeout: 15000 }),
            );
