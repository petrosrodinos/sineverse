...
export class AiService {
  ...
  async generateTextWithSchema<T>(
   ...
   */
  async generateVideo(
    options: AIGenerateVideoOptions,
    ...
    try {
      const videoModel = this.aiConfig.getVideoModelAdapter(
        options.provider,
        options.model,
      );
  ...
  async getVideoStatus(
    ...
    throw new Error(
      'Direct status check not implemented for blocking SDK calls. Use the background job lifecycle.',
    );
  }
}
