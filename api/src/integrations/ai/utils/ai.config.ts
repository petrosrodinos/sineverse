...
export class AiConfig {
    ...
    getVideoModelAdapter(provider: string, model: string) {
        switch (provider) {
            ...
            case AiProviders.openai:
                throw new Error('OpenAI Video (Sora) is not yet supported by the @ai-sdk/openai provider. Please use Gemini/Veo.');
            case AiProviders.runway:
            case AiProviders.pika:
                throw new Error(`Provider ${provider} is not yet integrated with the Vercel AI SDK.`);
            default:
                throw new Error(`Video provider ${provider} not supported by AI SDK yet.`);
        }
    }
