...
} from '../interfaces/stripe-payments.interface';
...
export class StripePaymentsService {
  ...
  async createCheckoutSession(
    payload: CreateStripeCheckoutSession,
          ...
          ? [
              {
                ...
                receipt_url: (paymentIntent.latest_charge as Stripe.Charge)
                  .receipt_url,
                payment_method: (paymentIntent.latest_charge as Stripe.Charge)
            ...
            ? [
                {
                  ...
                  payment_method: (pi.latest_charge as Stripe.Charge)
                    .payment_method as string | null,
                },
              ]
            : [],
  ...
  async getAccountChargeFees(charge_id: string, stripe_account_id?: string) {
    try {
      const accountCharge = await this.stripe.charges.retrieve(charge_id, {
