    ...
    try {
      switch (event.type) {
        ...
        case 'charge.succeeded': {
          ...
          if (
            charge.metadata?.context === StripePaymentContext.PROVIDER_PAYMENT
          ) {
          ...
          // Bookings flow
          if (
            charge.metadata?.context === StripePaymentContext.BOOKING_PAYMENT
          ) {
        ...
        case 'charge.updated':
          ...
          const { fee } =
            await this.stripePaymentsService.getBalanceTransaction(
              updatedCharge.balance_transaction,
            );
