...
function SocialProofStars({ className }: { className?: string }) {
  return (
    <div className={clsx("flex gap-0.5", className)} aria-hidden>
      {starIndices.map((s) => (
...
export function EstateLiftSocialProofCarousel({ items }: EstateLiftSocialProofCarouselProps) {
  ...
  return (
    <div className="mx-auto mt-12 w-full max-w-7xl" role="region" aria-label="Customer testimonials">
                    ...
                    <CardBody className="flex flex-1 flex-col gap-4 p-5 sm:gap-4 sm:p-5 lg:gap-5 lg:p-6">
                      ...
                      {item.highlight ? (
                        <div className="rounded-xl border border-primary/30 bg-primary/10 px-3 py-2.5 dark:bg-primary/15 sm:px-4 sm:py-3">
                          <p className="text-[10px] font-medium uppercase tracking-wider text-primary sm:text-xs">Outcome</p>
                          <p className="mt-0.5 text-xs font-semibold text-foreground sm:text-sm">{item.highlight}</p>
                        </div>
                      ) : null}
      ...
      <div className="mt-6 flex items-center justify-center gap-3 lg:hidden">
        <Button type="button" isIconOnly variant="bordered" radius="full" size="sm" aria-label="Previous testimonial" onPress={scrollPrev}>
          <ChevronLeft className="size-4" strokeWidth={2} />
        </Button>
        <Button type="button" isIconOnly variant="bordered" radius="full" size="sm" aria-label="Next testimonial" onPress={scrollNext}>
          <ChevronRight className="size-4" strokeWidth={2} />
        </Button>
      </div>

      <nav className="mt-6 flex flex-wrap items-center justify-center gap-2" aria-label="Testimonial slides">
        {items.map((item, index) => (
          <button
            ...
            className={clsx(
              ...
              selectedIndex === index ? "w-8 bg-primary shadow-sm shadow-primary/30" : "w-2 bg-default-300 hover:bg-default-400 dark:bg-default-600"
            )}
