...
import { Captions, Check, ImageIcon, Layers, Music, Smartphone, Sparkles, Star, TrendingUp, Video, Zap } from "lucide-react";
...
const openingAngles = [
  {
    ...
    body: "Your photos are not enough anymore. Listings with video get more views, more engagement, and more bookings. Turn your property photos into professional video tours in seconds — helping you stand out and convert faster. Upload. Customize. Download. Done.",
  },
  {
  ...
  },
  {
    ...
    body: "Sell properties faster with automated video tours. Create high-quality walkthrough videos from listing photos — without hiring a videographer. Impress clients, increase engagement, and close deals quicker.",
  },
] as const;

const problemIntro = "Most listings rely on static photos. But travelers and buyers want to experience a space — not just scroll through it.";

const problemPoints = ["Photos do not tell a story", "Videos are expensive and time-consuming", "Editing requires skills most hosts do not have"] as const;

const solutionPoints = ["AI generates smooth walkthrough motion", "Adds transitions, camera movement, and depth", "Ready for Airbnb, booking platforms, or social media"] as const;
...
const benefitLines = ["Listings with video can increase engagement by up to 3x.", "More bookings on Airbnb", "Higher perceived property value", "Better conversion rates for real estate agents"] as const;
...
const socialProofFeatured = {
  quote: "I dropped a walkthrough on my listing and inquiries doubled within two weeks. The whole thing took less time than editing one photo set.",
  name: "Marta Kowalski",
  role: "Superhost",
...
function SocialProofStars({ className }: { className?: string }) {
  return (
    <div aria-hidden className={`flex gap-0.5 ${className ?? ""}`}>
      {starIndices.map((s) => (
        <Star key={s} className="size-4 fill-primary/90 text-primary" strokeWidth={0} />
      ))}
    </div>
  );
}
    ...
    <LandingHashScroll pathnameMatch="/estatelift">
      <div className="min-h-screen bg-background">
        <section className="relative overflow-hidden border-b border-divider/40">
          ...
          <div className="relative mx-auto max-w-7xl px-6 pb-20 pt-16 md:pb-28 md:pt-24">
            <div className="mx-auto max-w-3xl text-center">
              <p className="mb-4 text-sm font-medium uppercase tracking-[0.2em] text-primary">Property video tours</p>
              <h1 className="text-balance text-4xl font-semibold leading-tight tracking-tight text-foreground md:text-5xl lg:text-6xl">Turn Your Property Photos Into Stunning Video Tours in Seconds</h1>
              <p className="mx-auto mt-6 max-w-2xl text-pretty text-lg text-default-500 md:text-xl">Upload your Airbnb or real estate photos and instantly create cinematic walkthrough videos that boost bookings and sales.</p>
              <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
                <EstateLiftCreateVideoLink className="min-w-[240px] font-semibold shadow-lg shadow-primary/25" label="Create Your First Video" />
              </div>
              <p className="mt-4 text-sm text-default-500">No editing skills required</p>
            </div>
            <div className="mx-auto mt-16 grid gap-6 md:grid-cols-3">
              {openingAngles.map((block) => (
                <Card key={block.title} className="border border-divider/50 bg-content1/30 shadow-none backdrop-blur-sm transition-transform duration-200 hover:scale-[1.02] dark:bg-content1/20">
                  <CardBody className="flex flex-col gap-3 p-6">
                    ...
                    <p className="text-sm leading-relaxed text-default-500">{block.body}</p>
                  </CardBody>
                </Card>
              ))}
            </div>
        ...
        <section className="border-b border-divider/40 py-20 md:py-28">
          <div className="mx-auto max-w-7xl px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-3xl font-semibold tracking-tight text-foreground md:text-4xl">The problem with static listings</h2>
            ...
            <ul className="mx-auto mt-12 flex max-w-2xl flex-col gap-4">
              {problemPoints.map((line) => (
                <li key={line} className="flex gap-3 rounded-2xl border border-divider/50 bg-content1/20 px-5 py-4 text-foreground dark:bg-content1/10">
                  <span className="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-lg bg-default-100 text-xs font-bold text-default-600">—</span>
        ...
        <section className="border-b border-divider/40 py-20 md:py-28">
          <div className="mx-auto max-w-7xl px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-3xl font-semibold tracking-tight text-foreground md:text-4xl">The solution</h2>
              <p className="mt-4 text-lg text-default-500">We turn your photos into a professional video tour — automatically.</p>
            </div>
            <ul className="mx-auto mt-12 grid max-w-3xl gap-4 md:grid-cols-1">
              {solutionPoints.map((line) => (
                <li key={line} className="flex gap-3 rounded-2xl border border-primary/20 bg-primary/5 px-5 py-4 text-foreground dark:bg-primary/10">
        ...
        <section className="scroll-mt-24 border-b border-divider/40 py-20 md:py-28" id="features">
          <div className="mx-auto max-w-7xl px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-3xl font-semibold tracking-tight text-foreground md:text-4xl">Key features</h2>
              <p className="mt-4 text-lg text-default-500">Everything you need to ship a tour in one flow.</p>
            </div>
        ...
        <section className="relative scroll-mt-24 overflow-hidden border-b border-divider/40 py-20 md:py-28" id="about">
          <div aria-hidden className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_50%_at_50%_100%,hsl(var(--heroui-primary-500)/0.08),transparent_60%)]" />
          <div className="relative mx-auto max-w-7xl px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="mt-3 text-3xl font-semibold tracking-tight text-foreground md:text-4xl">Trusted by hosts, managers, and agents</h2>
              <p className="mt-4 text-lg text-default-500">Real outcomes from people who ship listings every week—not generic praise.</p>
          ...
          <div className="mx-auto max-w-7xl px-6">
            <div className="flex flex-col items-center justify-between gap-10 rounded-2xl border border-divider/50 bg-gradient-to-br from-primary/10 via-background to-background p-10 md:flex-row md:p-14 lg:p-16">
              <div className="max-w-xl text-center md:text-left">
                ...
                <h2 className="text-3xl font-semibold tracking-tight text-foreground md:text-4xl">Turn your listing into a cinematic experience</h2>
                <p className="mt-4 text-lg text-default-500">Create your first video now.</p>
              </div>
              <EstateLiftCreateVideoLink className="shrink-0 min-w-[240px] font-semibold shadow-lg shadow-primary/25" label="Create your first video now" />
            </div>
          </div>
        </section>
        ...
        <footer className="border-t border-divider/50 bg-content1/40 backdrop-blur-sm dark:bg-content1/20">
          <div className="mx-auto max-w-7xl px-6 py-8 text-sm text-default-500 md:py-10">
            <p>
              Powered by{" "}
              <NextLink className="text-default-600 transition-colors hover:text-primary" href="https://logiqdev.com/" rel="noopener noreferrer" target="_blank">
                Logiqdev
              </NextLink>
            </p>
          </div>
