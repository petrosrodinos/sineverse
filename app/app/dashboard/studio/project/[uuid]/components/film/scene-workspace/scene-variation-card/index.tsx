...
export function SceneVariationCard({
  ...
  const { data: allAssetsResponse } = useProjectAssets(projectAssetsQuery, {
    enabled: !!variation?.uuid && !!isExpanded,
    refetchInterval: (query: any) => {
      const hasProcessing = query.state?.data?.data?.some(
        (a: any) =>
          a.status === ProjectAssetStatuses.PROCESSING ||
          a.status === ProjectAssetStatuses.PENDING,
      );
  ...
  const videoAssets = allAssets.filter(
    (a: any) => a.type === ProjectAssetTypes.VIDEO,
  );

  const promptImageAssets = allAssets.filter(
    (a: any) => a.role === AssetRoles.PROMPT_IMAGE,
  );

  const defaultKey = (
    videoAssets.find((a: any) => a.selected) || videoAssets[0]
  )?.uuid;
      ...
      <div className="flex items-center justify-between w-full">
        <div>
          ...
          <p className="text-xs text-default-500">
            Edit prompt and generation settings.
          </p>
        </div>
        <div>
          <Button
