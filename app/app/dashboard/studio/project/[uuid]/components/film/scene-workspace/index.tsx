...
import {
  useDeleteSceneVariation,
  useSceneVariations,
  useDuplicateSceneVariation,
  useUpdateSceneVariation,
} from "@/features/scene-variations/hooks/use-scene-variations";
...
export function SceneWorkspace({}: SceneWorkspaceProps) {
  ...
  const { data: scene_variations, isLoading } = useSceneVariations(
    variationsQuery,
    { enabled: !!sceneUuid },
  );

  const [variationToDelete, setVariationToDelete] =
    useState<SceneVariation | null>(null);

  const [variationToDuplicate, setVariationToDuplicate] =
    useState<SceneVariation | null>(null);

  const [variationToEdit, setVariationToEdit] = useState<SceneVariation | null>(
    null,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-default-500 mt-1">
            Configure the specific settings for these video variations to tune
            the scene generation exactly as needed.
          </p>
        </div>
      ...
      >
        {[
          ...(scene_variations || []).map(
            (variation: SceneVariation, index: number) => (
              <AccordionItem
                ...
                subtitle={
                  <span className="text-xs text-default-400">
                    {StylesOptionsLabels[
                      variation.project_assets?.find(
                        (a: any) => a.role === AssetRoles.GENERATED_VIDEO,
                    ...
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 z-20 flex items-center gap-3">
                      <Switch
                        ...
                        onValueChange={(isSelected) =>
                          handleToggleSelected(variation, isSelected)
                        }
                      >
                        Selected
                      </Switch>
              ...
              >
                <SceneVariationCard
                  isExpanded={
                    selectedKeys === "all" ||
                    selectedKeys.has(`variation-${variation.uuid}`)
                  }
                  variation={variation}
                />
              </AccordionItem>
