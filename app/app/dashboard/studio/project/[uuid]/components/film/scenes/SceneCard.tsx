      ...
      <div className="p-3 space-y-2 text-left w-full flex flex-col items-start min-w-0">
        <div className="flex items-center justify-between gap-1 w-full">
          <div className="flex items-center gap-2">
            {isEditMode && (
              <GripVertical className="size-4 text-default-400 cursor-grab active:cursor-grabbing" />
            )}
          ...
          {!isEditMode ? (
            scene.scene_variations !== undefined && (
              <span className="text-[10px] font-medium text-default-500 border border-default-200 bg-default-50 px-2 py-0.5 rounded-full dark:border-default-100 dark:bg-default-200">
                {scene.scene_variations.length}{" "}
                {scene.scene_variations.length === 1
                  ? "variation"
                  : "variations"}
              </span>
            )
        ...
        <p className="text-sm font-medium text-foreground line-clamp-1 w-full text-left">
          {scene.title || "Untitled"}
        </p>
        {scene.description && (
          <p className="text-xs text-default-500 line-clamp-2 w-full text-left">
            {scene.description}
          </p>
        )}
      </div>
