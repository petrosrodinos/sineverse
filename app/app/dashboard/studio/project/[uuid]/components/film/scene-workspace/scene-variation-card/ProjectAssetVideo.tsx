...
export default function ProjectAssetVideo({
  asset,
  variation,
  promptImageAssets,
  config,
  isEnriched,
  handleClose,
}: ProjectAssetVideoProps) {
  const [negativeOpen, setNegativeOpen] = useState(false);

  const [isConfirmOpen, setIsConfirmOpen] = useState(false);

  const [enrichedVariation, setEnrichedVariation] =
    useState<VideoGenerationConfig | null>(null);
  ...
  const [pendingGenerateConfig, setPendingGenerateConfig] = useState<
    any | null
  >(null);
  ...
  const { data: polledVideo } = useProjectAsset(asset?.uuid || "", {
    refetchInterval: (query: any) => {
      ...
      return status === ProjectAssetStatuses.PROCESSING ||
  ...
  useEffect(() => {
    ...
    const isNowDone =
      polledVideo?.status === ProjectAssetStatuses.COMPLETED ||
      polledVideo?.status === ProjectAssetStatuses.FAILED;
  ...
  const isImageToVideoModel =
    editedConfig.ai_model?.includes("image-to-video") ||
    editedConfig.ai_model?.includes("i2v");

  const latestPromptImageUuid =
    promptImageAssets && promptImageAssets.length > 0
      ? promptImageAssets[0].uuid
      : undefined;
  ...
  const validateVariation = () => {
    ...
    if (
      ...
      addToast({
        ...
        description:
          "Please upload or configure an image for image-to-video models.",
        severity: "danger",
      });
  ...
  const handleSave = async () => {
    ...
    let finalPromptImageUuids: string[] =
      currentAssetImages.length > 0
        ? currentAssetImages.map((img: any) => img.uuid)
        : !isEnriched && latestPromptImageUuid
          ? [latestPromptImageUuid]
          : [];
    ...
    try {
      if (isEnriched) {
        ...
        if (pendingUploadFile) {
          const asset = await uploadImageMutation.mutateAsync({
            uuid: variation.uuid,
      ...
      addToast({
        ...
        description:
          error.message ||
          "An error occurred while creating the video iteration.",
        severity: "danger",
      });
    ...
    <div className="flex flex-col gap-6 w-full pb-4">
      ...
      {!isEnriched &&
        (displayVideoStatus === ProjectAssetStatuses.PROCESSING ||
          displayVideoStatus === ProjectAssetStatuses.PENDING) && (
      ...
      {!isEnriched &&
        ...
        displayVideo?.document?.url && (
          <div className="flex flex-col gap-3">
            <div
              className={`w-full aspect-video rounded-xl overflow-hidden bg-black shadow-lg ring-1 transition-all ${isSelected ? "ring-primary border border-primary/50" : "ring-default-200"}`}
            >
              <video
                controls
                ...
                poster={
                  displayVideo?.document?.url
                    ? undefined
                    : (displayVideo as any).prompt_images?.[0]?.document?.url ||
                      promptImageAssets?.[0]?.document?.url
                }
      ...
      <div className="flex flex-col gap-4">
        <ExpandableTextarea
          ...
          onValueChange={(val: string) =>
            handleConfigChange("prompt_text", val)
          }
        />
        ...
        <Accordion
          className="px-0 gap-0 border border-default-200 dark:border-default-100/20 rounded-xl overflow-hidden"
          selectedKeys={negativeOpen ? ["negative"] : []}
          onSelectionChange={(k) =>
            setNegativeOpen(Array.from(k).includes("negative"))
          }
        >
          <AccordionItem
            key="negative"
            ...
            title={<span className="text-sm font-medium">Negative Prompt</span>}
          >
            <Textarea
              ...
              variant="bordered"
              onValueChange={(val) =>
                handleConfigChange("negative_prompt", val)
              }
            />
          </AccordionItem>
        </Accordion>
          ...
          {isEnriched ? (
            ...
            <>
              <Button
                ...
                isDisabled={
                  !variation?.uuid ||
                  !variation?.scene_uuid ||
                  !editedConfig.ai_model ||
                  displayVideoStatus === ProjectAssetStatuses.PROCESSING ||
                  displayVideoStatus === ProjectAssetStatuses.PENDING
                }
