...
import { ESTATE_VIDEO_MODEL_OPTIONS, ESTATE_VISITOR_MAX_PROMPT_IMAGES } from "../../../../../../../../../config/dropdowns/project/estate-workflow.constants";
...
export function UploadPhotosStep({ pendingFiles, setPendingFiles, selectedVideoModelId, onVideoModelChange }: UploadPhotosStepProps) {
  ...
  const visitorPhotoCapReached = isVisitor && totalPhotoCount >= ESTATE_VISITOR_MAX_PROMPT_IMAGES;
      ...
      if (isVisitor) {
        ...
        if (images.length > remaining) {
          ...
          setVisitorPhotoLimitModal({
            ...
            body: `Preview visits can include up to ${cap} listing photos. We added the first ${toAdd.length} from your selection (${skipped} not added). Create a free account to upload more.`,
          });
  ...
  return (
    <div className="flex flex-col gap-6">
      <input ref={inputRef} multiple accept={ACCEPTED_IMAGE_INPUT} className="hidden" type="file" onChange={handleFileChange} />

      <div className={pickerClassName} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}>
        <div className="p-4 sm:p-5">
          {showLoadingGrid ? (
            ...
            <button className="flex min-h-[200px] w-full cursor-pointer flex-col items-center justify-center gap-4 rounded-xl px-4 py-10 text-center transition-colors duration-200 hover:bg-secondary-500/[0.06] focus:outline-none focus-visible:ring-2 focus-visible:ring-secondary-500 focus-visible:ring-offset-2 focus-visible:ring-offset-background" type="button" onClick={openPicker}>
              ...
              <div className="max-w-sm space-y-1.5">
                ...
                <p className="text-small leading-relaxed text-default-500">{isVisitor ? `Drop images here or click to browse. You can add up to ${ESTATE_VISITOR_MAX_PROMPT_IMAGES} photos on a preview visit.` : "Drop images here or click to browse. You can add more anytime."}</p>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-tiny text-default-500">
                      ...
                      {isVisitor ? (visitorPhotoCapReached ? ` · ${ESTATE_VISITOR_MAX_PROMPT_IMAGES} of ${ESTATE_VISITOR_MAX_PROMPT_IMAGES} (preview limit)` : ` · up to ${ESTATE_VISITOR_MAX_PROMPT_IMAGES} on preview · drop to add more`) : " · drop to add more"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                {promptImageAssets.map((asset) => (
                  <div key={asset.uuid} className="group relative aspect-[4/3] overflow-hidden rounded-xl bg-default-200/40 shadow-sm ring-1 ring-black/[0.06] transition duration-200 hover:ring-secondary-500/35 dark:bg-default-100/10 dark:ring-white/10">
                    {asset.status === ProjectAssetStatuses.PROCESSING ? <Skeleton className="h-full w-full rounded-xl" /> : <img alt="" className="h-full w-full object-cover" src={asset.document.url} />}
                    ...
                    <button aria-label="Remove image" className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-background/90 text-foreground shadow-md backdrop-blur-sm transition hover:bg-danger/90 hover:text-white md:opacity-0 md:group-hover:opacity-100" type="button" onClick={handleRemoveApiAsset(asset)}>
                ...
                {pendingFiles.map((pf) => (
                  <div key={pf.id} className="group relative aspect-[4/3] overflow-hidden rounded-xl bg-default-200/40 shadow-sm ring-1 ring-secondary-400/40 transition duration-200 dark:bg-default-100/10">
                    <img alt="" className="h-full w-full object-cover" src={pf.previewUrl} />
                    <button aria-label="Remove image" className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-background/90 text-foreground shadow-md backdrop-blur-sm transition hover:bg-danger/90 hover:text-white md:opacity-0 md:group-hover:opacity-100" type="button" onClick={handleRemovePendingFile(pf.id)}>
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                ))}
      ...
      {isAdmin && hasPhotos && (
        <Select classNames={{ trigger: "min-h-10" }} label="Walkthrough model" selectedKeys={new Set([selectedVideoModelId])} size="sm" onSelectionChange={handleVideoModelChange}>
          {ESTATE_VIDEO_MODEL_OPTIONS.map((option) => (
            <SelectItem key={option.id}>{`${option.label} - $${option.price.toFixed(3)}/sec`}</SelectItem>
          ))}
        </Select>
      )}

      <ConfirmationModal confirmText="Remove" description={confirmModalDescription} isLoading={isDeletingScene} isOpen={isConfirmModalOpen} title="Remove photo" onClose={handleCloseRemoveModal} onConfirm={handleConfirmRemove} />

      <Modal isOpen={visitorPhotoLimitModal.open} placement="center" onOpenChange={(open) => setVisitorPhotoLimitModal((prev) => ({ ...prev, open }))}>
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">{visitorPhotoLimitModal.title}</ModalHeader>
          <ModalBody>
            <p className="text-sm leading-relaxed text-default-600">{visitorPhotoLimitModal.body}</p>
          </ModalBody>
          <ModalFooter className="flex flex-wrap gap-2">
            <Button variant="flat" onPress={() => setVisitorPhotoLimitModal((prev) => ({ ...prev, open: false }))}>
              Got it
            </Button>
            <Button as={NextLink} className="font-semibold" color="primary" href={Routes.auth.sign_up}>
              Create free account
            </Button>
          </ModalFooter>
        </ModalContent>
