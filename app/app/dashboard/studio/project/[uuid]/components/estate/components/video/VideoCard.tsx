...
export function VideoCard({ asset, compact = false, videoAssetUuid }: VideoCardProps) {
  ...
  const trimRange = useEstateWorkflowStore(
    (s) => s.trimRangeByVideoUuid[videoAssetUuid] ?? { start: 0, end: 5 },
  );
  const transitionId = useEstateWorkflowStore(
    (s) => s.transitionByVideoUuid[videoAssetUuid] ?? ESTATE_DEFAULT_TRANSITION_ID,
  );
  const audioTrackId = useEstateWorkflowStore(
    (s) => s.estateAudioTrackByVideoUuid[videoAssetUuid] ?? ESTATE_DEFAULT_AUDIO_TRACK_ID,
  );
  ...
  const playBtnShell = compact
    ? "flex h-9 w-9 items-center justify-center rounded-full bg-background/80 text-foreground shadow-md backdrop-blur-sm"
    : "flex h-14 w-14 items-center justify-center rounded-full bg-background/80 text-foreground shadow-lg backdrop-blur-sm";
      ...
      <CardBody className={compact ? "gap-2 p-3" : "gap-4 p-4"}>
        <div className="flex flex-wrap items-center justify-between gap-1">
          <span
            className={
              compact
                ? "text-tiny font-medium text-default-600"
                : "text-small font-medium text-default-600"
            }
          >
            scene.order {asset.scene.order}
          </span>
        ...
        {showEditor && (
          <div className={compact ? "flex flex-col gap-2" : "flex flex-col gap-4"}>
            ...
            <Select
              label="Transition"
              size="sm"
              selectedKeys={new Set([transitionId])}
