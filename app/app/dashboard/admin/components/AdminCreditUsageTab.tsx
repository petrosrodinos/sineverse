...
export function AdminCreditUsageTab({ isActive }: AdminCreditUsageTabProps) {
  ...
  const [creditUsageQuery, setCreditUsageQuery] =
    useState<AdminCreditUsageQuery>({
      page: 1,
      limit: 10,
      sort_by: "created_at",
      sort_order: "desc",
  ...
  const { data: creditUsageData, isLoading: creditUsageLoading } =
    useAdminCreditUsage(creditUsageQuery, { enabled: isActive });

  const { data: usersData } = useAdminUsers(usersQuery, { enabled: isActive });

  const userFilterOptions = useMemo(
    () => [
      { uuid: "all", email: "All users" },
  ...
  const creditUsageControls = (
    <div className="grid w-full min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 sm:items-end lg:flex lg:w-auto lg:min-w-0 lg:flex-1 lg:flex-wrap lg:justify-end lg:gap-2">
      ...
      >
        ...
        <SelectItem key="provider_charge_amount_usd">
          Provider Charge USD
        </SelectItem>
  ...
  return (
    <div className="mt-4">
      <AdminDataTable<AdminCreditUsageRow>
        ...
        onPageChange={(page) =>
          setCreditUsageQuery((prev) => ({ ...prev, page }))
        }
      />
    </div>
  );
