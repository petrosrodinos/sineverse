...
export function AdminOverviewTab({ isActive }: AdminOverviewTabProps) {
  ...
  const usageCards = useMemo(
    () => [
      ...
      {
        label: "Total Final Projects",
        value: (overview?.total_final_projects ?? 0).toLocaleString(),
      },
      {
        label: "Total Videos Created",
        value: (overview?.total_videos_created ?? 0).toLocaleString(),
      },
      {
        label: "Total Images Created",
        value: (overview?.total_images_created ?? 0).toLocaleString(),
      },
      {
        label: "Total Token Usage",
        value: (overview?.total_token_usage ?? 0).toLocaleString(),
      },
    ],
    [overview],
  );
    ...
    () => [
      {
        ...
        value: formatCurrencyFromCents(
          overview?.total_gross_revenue_cents ?? 0,
        ),
      },
      {
        label: "Total Net Revenue",
        value: formatCurrencyFromCents(overview?.total_net_revenue_cents ?? 0),
      },
      {
  ...
  return (
    <Tabs aria-label="Overview metrics" className="mt-4" variant="underlined">
