...
export function DashboardSidebar({ items }: DashboardSidebarProps) {
  ...
  const visibleItems = items.filter(
    (item) =>
      !item.roles ||
      (session?.role && item.roles.includes(session.role as RoleType)),
  );
  ...
  const sidebarContent = (
    <nav className="flex h-full w-full flex-col rounded-2xl border border-default-200 bg-default-100 py-1.5 shadow-lg shadow-default-200/20 dark:border-default-100/10 dark:bg-default-50 dark:shadow-black/10">
      <div
        className={clsx(
          "flex items-center border-b border-default-200/60 dark:border-default-100/20 pb-3 mb-2",
            ...
            <NextLink
              ...
              className={clsx(
                ...
                isActive
                  ...
                  : "text-default-600 hover:bg-default-100 hover:text-default-900 dark:text-default-400 dark:hover:bg-default-100/20 dark:hover:text-default-100",
              )}
              href={item.href}
            >
      ...
      <div className="mt-auto border-t border-default-200/60 dark:border-default-100/20 pt-3 px-2">
        <Dropdown isDisabled={isSessionLoading} placement="top-start">
          <DropdownTrigger>
            <Button
              className={clsx(
            ...
            >
              {isSessionLoading ? (
                ...
                <>
                  <Avatar className="h-9 w-9 shrink-0" name={full_name} />
                  {!collapsed && (
                    <>
    ...
    <>
      ...
      >
        <DrawerContent className="bg-transparent shadow-none w-72 max-w-[80vw]">
          <DrawerBody className="p-0 px-2 py-3">{sidebarContent}</DrawerBody>
        </DrawerContent>
      </Drawer>
    </>
  );
