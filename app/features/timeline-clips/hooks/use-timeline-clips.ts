...
export const useTimelineClips = (
  ...
  options?: Omit<
    UseQueryOptions<TimelineClip[], Error, TimelineClip[], any>,
    "queryKey" | "queryFn"
  >,
) => {
  ...
  return useMutation<TimelineClip, Error, CreateTimelineClipDto>({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: [
          "timeline-clips",
          { final_project_uuid: data.final_project_uuid },
        ],
      });
    },
    onError: (error) => {
...
export const useUpdateTimelineClip = () => {
  ...
  return useMutation<
    ...
    Error,
    {
      ...
      invalidateTimelineQueries?: boolean;
    }
  >({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.timelineClip(data.uuid)],
      });
    },
    onError: (error) => {
      addToast({
        title: "Failed to save clip settings",
        description: error.message,
        severity: "danger",
      });
    },
