...
const QueryKeys = {
  timelineCaptions: (query: TimelineCaptionQueryDto) => [
    "timeline-captions",
    query,
  ],
};
...
export const useTimelineCaptions = (
  ...
  options?: Omit<
    UseQueryOptions<TimelineCaption[], Error, TimelineCaption[], any>,
    "queryKey" | "queryFn"
  >,
) => {
  ...
  return useMutation<TimelineCaption, Error, CreateTimelineCaptionDto>({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["timeline-captions", { clip_uuid: data.clip_uuid }],
      });
    },
    onError: (error) => {
      addToast({
        title: "Failed to create caption",
...
export const useUpdateTimelineCaption = () => {
  ...
  return useMutation<
    TimelineCaption,
    Error,
    { uuid: string; dto: UpdateTimelineCaptionDto; clip_uuid: string }
  >({
    mutationFn: ({ uuid, dto }) => updateTimelineCaption(uuid, dto),
    onSuccess: (_, { clip_uuid }) => {
      queryClient.invalidateQueries({
        queryKey: ["timeline-captions", { clip_uuid }],
      });
    },
    onError: (error) => {
      addToast({
...
export const useDeleteTimelineCaption = () => {
  ...
  return useMutation<void, Error, { uuid: string; clip_uuid: string }>({
    ...
    onError: (error) => {
      addToast({
        title: "Failed to delete caption",
        description: error.message,
        severity: "danger",
      });
    },
