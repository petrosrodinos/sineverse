...
export const useUpdateTimelineTransition = () => {
    ...
    return useMutation<TimelineTransition, Error, { uuid: string; dto: UpdateTimelineTransitionDto }>({
        mutationFn: ({ uuid, dto }) => updateTimelineTransition(uuid, dto),
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: [QueryKeys.timelineTransition(data.uuid)] });
        },
    });
};
...
export const useDeleteTimelineTransition = () => {
    return useMutation<void, Error, string>({
        ...
        onError: (error) => {
            addToast({ title: "Failed to delete transition", description: error.message, severity: "danger" });
        },
    });
};
