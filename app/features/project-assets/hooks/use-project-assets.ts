...
import {
  ...
  createSceneVideo,
  createProjectAsset,
  createSceneVariationImage,
  deleteProjectAsset,
  deleteSceneVariationPromptImage,
  uploadSceneVariationPromptImage,
  selectProjectAsset,
  enrichProjectAssetVideo,
  createEstateWalkthroughVideos,
} from "../services/project-assets.services";
...
export const useProjectAssets = (
  ...
  options?: Omit<
    UseQueryOptions<ProjectAssetsResponse, Error, ProjectAssetsResponse, any>,
    "queryKey" | "queryFn"
  >,
) => {
...
export const useProjectAsset = (
  uuid: string,
  options?: Omit<
    UseQueryOptions<ProjectAsset, Error, ProjectAsset, any>,
    "queryKey" | "queryFn"
  >,
) => {
...
export const useCreateEstateWalkthroughVideos = () => {
  ...
  return useMutation({
    mutationFn: (payload: CreateEstateWalkthroughVideosDto) =>
      createEstateWalkthroughVideos(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.projectAssets] });
    },
  });
...
export const useUploadSceneVariationPromptImage = () => {
  ...
  return useMutation({
    mutationFn: ({ uuid, file }: { uuid: string; file: File }) =>
      uploadSceneVariationPromptImage(uuid, file),
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.projectAsset(variables.uuid)],
      });
    },
  });
};
...
export const useCreateSceneVariationImage = () => {
  return useMutation({
    ...
    }) => createSceneVariationImage(uuid, payload),
...
export const useEnrichProjectAssetVideo = (uuid: string) => {
  ...
  return useMutation({
    ...
    }) => enrichProjectAssetVideo(uuid, enrichDto),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.projectAsset(uuid)],
      });
