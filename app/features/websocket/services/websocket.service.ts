...
const connectionListeners: Set<(state: WebsocketConnectionState) => void> =
  new Set();
...
const setupEventListeners = (): void => {
  ...
  socket.on(
    "connected",
    (data: { socket_id: string; account_uuid: string }) => {
      updateState({
    ...
    if (callbacks) {
      callbacks.forEach((callback) => {
        try {
          ...
          console.error(
            `[WebSocket] Error in callback for event "${event}":`,
            error,
          );
        }
      });
...
export const websocketUnsubscribe = (
  event: string,
