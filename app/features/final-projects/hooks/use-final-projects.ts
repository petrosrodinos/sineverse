...
import {
  ...
  deleteFinalProject,
  renderFinalProject,
  deleteAllFinalProjectVideos,
  deleteFinalProjectVideoByDocument,
} from "../services/final-projects.services";
import {
  FinalProject,
...
export const useFinalProjectByProject = (projectUuid: string) => {
  ...
  return {
    finalProject: result.data?.[0] ?? null,
...
export const useUpdateFinalProject = () => {
  ...
  return useMutation<
    FinalProject,
    Error,
    { uuid: string; finalProject: UpdateFinalProjectDto }
  >({
    mutationFn: ({ uuid, finalProject }) =>
      updateFinalProject(uuid, finalProject),
    onSuccess: (_, { uuid }) => {
      queryClient.invalidateQueries({ queryKey: ["final-projects"] });
...
export const useDeleteFinalProjectVideo = () => {
  ...
  return useMutation<void, Error, { finalProjectUuid: string }>({
    ...
    onSuccess: (_, { finalProjectUuid }) => {
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.finalProject(finalProjectUuid)],
      });
...
export const useDeleteFinalProjectVideoByDocument = () => {
  ...
  return useMutation<
    void,
    Error,
    { finalProjectUuid: string; documentUuid: string }
  >({
