...
} from "../interfaces/final-projects.interfaces";
...
export const getFinalProjects = async (
  ...
  try {
    const response = await axiosInstance.get<FinalProject[]>(
      ApiRoutes.final_projects.prefix,
      { params: query },
    );
    ...
    console.error(
      error?.response?.data?.message || "Failed to fetch final projects",
    );
...
export const getFinalProject = async (uuid: string): Promise<FinalProject> => {
  try {
    const response = await axiosInstance.get<FinalProject>(
      ApiRoutes.final_projects.final_project(uuid),
    );
...
export const createFinalProject = async (
  finalProject: CreateFinalProjectDto,
): Promise<FinalProject> => {
  try {
    const response = await axiosInstance.post<FinalProject>(
      ApiRoutes.final_projects.prefix,
      finalProject,
    );
    ...
    console.error(
      error?.response?.data?.message || "Failed to create final project",
    );
...
export const updateFinalProject = async (
  uuid: string,
  finalProject: UpdateFinalProjectDto,
): Promise<FinalProject> => {
  try {
    const response = await axiosInstance.patch<FinalProject>(
      ApiRoutes.final_projects.final_project(uuid),
      finalProject,
    );
...
export const downloadFinalProjectVideo = async (
  uuid: string,
): Promise<Blob> => {
  try {
    const response = await axiosInstance.get(
      ApiRoutes.final_projects.download(uuid),
      {
        responseType: "blob",
      },
    );
    ...
    console.error(
      error?.response?.data?.message || "Failed to download rendered video",
    );
...
export const downloadFinalProjectVideoByDocument = async (
  ...
  try {
    const response = await axiosInstance.get(
      ApiRoutes.final_projects.download_document(
        finalProjectUuid,
        documentUuid,
      ),
      {
...
export const deleteFinalProjectVideoByDocument = async (
  ...
  try {
    ...
    console.error(
      error?.response?.data?.message || "Failed to delete rendered video",
    );
...
export const deleteAllFinalProjectVideos = async (
  ...
  try {
    ...
    console.error(
      error?.response?.data?.message || "Failed to delete rendered videos",
    );
