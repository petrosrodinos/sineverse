import {
  ...
  GenerateAiScenesDto,
  ReorderScenesDto,
  CreateEstateScenesFromImagesDto,
} from "../interfaces/scenes.interfaces";
...
export const createScene = async (scene: CreateSceneDto): Promise<Scene> => {
  try {
    const response = await axiosInstance.post<Scene>(
      ApiRoutes.scenes.prefix,
      scene,
    );
...
export const updateScene = async (
  uuid: string,
  scene: UpdateSceneDto,
): Promise<Scene> => {
  try {
    const response = await axiosInstance.patch<Scene>(
      ApiRoutes.scenes.scene(uuid),
      scene,
    );
...
export const generateAiScenes = async (
  scene: GenerateAiScenesDto,
): Promise<Scene[]> => {
  try {
    const response = await axiosInstance.post<Scene[]>(
      ApiRoutes.scenes.generate_ai_scenes,
      scene,
    );
    ...
    console.error(
      error?.response?.data?.message || "Failed to generate scenes",
    );
...
export const createEstateScenesFromImages = async (
  payload: CreateEstateScenesFromImagesDto,
): Promise<Scene[]> => {
  try {
    ...
    const response = await axiosInstance.post<Scene[]>(
      ApiRoutes.scenes.estate_from_images,
      formData,
      {
        headers: {
