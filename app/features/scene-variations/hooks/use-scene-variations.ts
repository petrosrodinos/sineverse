...
import {
  ...
  duplicateSceneVariation,
} from "../services/scene-variations.services";
import {
  SceneVariation,
...
export const useSceneVariations = (
  ...
  return useQuery<SceneVariation[]>({
    queryKey: [QueryKeys.sceneVariations, query],
    queryFn: () => getSceneVariations(query),
    ...options,
  });
};
...
export const useSceneVariation = (uuid: string, options?: any) => {
  return useQuery<SceneVariation>({
    queryKey: [QueryKeys.sceneVariation(uuid)],
    queryFn: () => getSceneVariation(uuid),
    ...options,
  });
};
...
export const useUpdateSceneVariation = () => {
  ...
  return useMutation<
    SceneVariation,
    Error,
    { uuid: string; sceneVariation: UpdateSceneVariationDto }
  >({
    mutationFn: ({ uuid, sceneVariation }) =>
      updateSceneVariation(uuid, sceneVariation),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.sceneVariations] });
